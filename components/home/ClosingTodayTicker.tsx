  ...
  return (
    <div className="bg-cream-dark border-b border-warm-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex items-center gap-3 overflow-x-auto">
        <span className="shrink-0 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide bg-accent text-white rounded-full px-2 py-0.5">
          <T bn="শেষ হচ্ছে" en="Ending Soon" />
        </span>
        <div className="flex items-center gap-4 overflow-x-auto text-ink">
          {posts.map((p, i) => {
            ...
            return (
              ...
              >
                {i > 0 && <span className="text-warm-muted">•</span>}
                <span className="max-w-[160px] sm:max-w-xs truncate">{title}</span>
                <span className="font-bold bg-accent/15 text-accent-dark rounded-full px-1.5 py-0.5 text-[10px]">
        ...
        <Link href="/jobs?deadlineWithinDays=3" className="ml-auto shrink-0 text-xs sm:text-sm font-semibold text-primary-700 hover:underline whitespace-nowrap">
