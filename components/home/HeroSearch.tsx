...
const POPULAR_SEARCHES = ['BPSC', 'Bank', 'Police', 'NBR', 'Teacher', 'Engineer', 'IT', 'Army'];
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-10">
          ...
          <div className="flex-1 min-w-0">
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-2">
              ...
              {categoryTypes.length > 0 && (
                <select
                  ...
                  className="px-3 py-3 rounded-xl text-gray-900 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-accent shadow-md sm:max-w-[160px]"
            ...
            <div className="flex items-center gap-1.5 flex-wrap mt-3">
              <span className="text-xs text-white/60 mr-0.5">{t('জনপ্রিয় অনুসন্ধান:', 'Popular Searches:')}</span>
              {POPULAR_SEARCHES.map((term) => (
                <button
                  key={term}
                  type="button"
                  onClick={() => goSearch(term)}
                  className="px-2.5 py-1 rounded-full bg-white/10 hover:bg-white/20 border border-white/10 text-xs transition-colors"
                >
                  {term}
                </button>
              ))}
