...
import {
  ...
  adminRejectInstituteReview,
  adminDeleteInstituteReview,
} from '@/lib/api';
import type { AdminInstituteReview } from '@/lib/types';

function Stars({ rating }: { rating: number }) {
  return (
    <span className="text-amber-500 text-xs tracking-tight" aria-label={`${rating}/5`}>
      {'★'.repeat(rating)}
...
export default function AdminInstituteReviewsPage() {
  const router = useRouter();
  const [token, setToken] = useState('');
  const [adminName, setAdminName] = useState('');
  const [items, setItems] = useState<AdminInstituteReview[]>([]);
  ...
  const load = useCallback(async (t: string, f: string, p: number) => {
    setLoading(true);
    try {
      const res = await adminGetInstituteReviews(t, f, p, 20);
  ...
  const handleApprove = async (id: number) => {
    ...
    try {
      await adminApproveInstituteReview(token, id);
  ...
  const handleReject = async (id: number) => {
    try {
      await adminRejectInstituteReview(token, id, rejectNote || undefined);
      flash('বাতিল হয়েছে');
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('রিভিউটি স্থায়ীভাবে মুছে ফেলবেন?')) return;
    try {
      await adminDeleteInstituteReview(token, id);
  ...
  return (
    <AdminShell title="ইনস্টিটিউট রিভিউ" subtitle="ব্যবহারকারীর রিভিউ মডারেশন" adminName={adminName} token={token}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        {msg && (
          <div className="mb-4 bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-3 text-sm">{msg}</div>
        )}

        <PageHeader title="ইনস্টিটিউট রিভিউ মডারেশন" />
        ...
        <div className="bg-white rounded-2xl border border-warm-border p-5 space-y-4">
          ...
          {loading ? (
            ...
            <p className="text-sm text-warm-muted py-4 text-center">কোনো রিভিউ নেই</p>
          ) : (
                  ...
                  <div className="flex items-start gap-3 flex-wrap">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-warm-muted">
                        {item.userName} <span className="font-normal">({item.userEmail})</span> • {item.instituteName}
                      </p>
                    </div>
